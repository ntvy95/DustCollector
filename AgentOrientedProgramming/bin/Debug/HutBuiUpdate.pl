...
useful(X,Y) :- (A is 0;A is 90;A is 180;A is 270), Temp1 is X + round(cos(pi*A/180)), Temp2 is Y + round(sin(pi*A/180)), not(wasweight(Temp1,Temp2,_)), not(wasweight(Temp1, Temp2, inf)).
